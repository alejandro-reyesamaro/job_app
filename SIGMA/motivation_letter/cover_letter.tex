...
{{\bf SIGMA}\\France\\
\vspace{0.2in}
\textbf{Object :} Candidature au poste \textit{Lead Développeur .NET H/F}
}
...
Je vous écris concernant l'offre d'emploi publié sur le site web de votre entreprise : \textbf{Lead Développeur .NET H/F}. J'occupe actuellement le poste d'Attaché temporaire d'enseignement et de recherche (ATER) dans le Laboratoire de Sciences du Numérique de l'Université de Nantes (LS2N). Je viens de soutenir ma thèse de doctorat et j'aimerais continuer ma vie professionnelle dans un contexte plus orienté au développement de logicielles.

Au cours de mon doctorat, j'ai con\c cu et implémenté un framework de haut niveau pour la création de solveurs de problèmes combinatoires en parallèle, en C++. Avant ma thèse, mon expérience m'a amené à principalement utiliser les technologies .NET et le langage C\#. J'ai fait partie de plusieurs groupes de développement et de recherche pour la création de logiciels. J'ai aussi dirigé des projets de développement (\textit{front--end} et/ou \textit{back--end}). La plupart de ces projets présentent une couche de données (\textit{data layer}), basée sur du SQL Server.
