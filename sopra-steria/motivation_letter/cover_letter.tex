...
\begin{letter}
{{\bf Sopra Steria}\\Nantes, France\\
\vspace{0.2in}
\textbf{Object :} Candidature au poste \textit{Ingénieur Études et Développement .Net H/F}
}
...
Je vous écris concernant l'offre d'emploi publié sur le site web de votre entreprise : \textbf{Ingénieur Études et Développement .Net H/F}. J'occupe actuellement le poste d'Attaché Temporaire d'Enseignement et de Recherche (ATER) dans le Laboratoire de Sciences du Numérique de l'Université de Nantes (LS2N). Je viens de soutenir ma thèse de doctorat et je souhaite continuer ma vie professionnelle dans un contexte orienté développement logicielles.

Au cours de mon doctorat, j'ai con\c cu et implémenté un framework de haut niveau pour la création de solveurs de problèmes combinatoires en parallèle, en C++. Avant ma thèse, mon expérience m'a amené principalement à utiliser les technologies .NET de Microsoft et le langage C\#. 

J'ai fait partie de plusieurs groupes de développement \tet{dans des entreprises}, et de recherche \tet{dans des universités à La Havane, les Iles Baleares et Nantes}, pour la création de logiciels. \tet{En tant que developpeur \textit{freelance}}, j'ai aussi crée et dirigé des projets de développement (\textit{front--end} et/ou \textit{back--end}), \tet{étant en charge de toutes les étapes des projets, y compris la} rédaction détaillée des spécifications fonctionnelles et techniques. \tet{Dans mon CV je résume les caracteristiques de \textit{Neurohipot} et \textit{POSL}, des logiciels que j'ai conçu et développé. Un des projets en tant que développeur dans \textit{SCG} à été la conception et développement, de toutes pièces, d'un site web pour l'évaluation des niveaux de responsabilité sociale des petites entreprises à Costa Rica, ou j'ai aussi conçu le dessin web.} 
...
Il vaut la peine d'ajouter aussi que en tant qu'enseignant aux Universités de la Havane et de Nantes, j'ai enseigné pendant des années les langages C/C++/C\#, ainsi que la technologie \textit{Message Passing Interface}, entre autres.

Votre offre d'emploi est une grande opportunité d'appliquer toutes mes connaissances et mon expérience de développeur. J'y vois aussi l'opportunité d'évoluer dans ma carrière professionnelle au sein de votre entreprise. Je suis convaincu que mon expérience et ma formation comme développeur dans diverses entreprises, ainsi que mes années de travail en équipe dans plusieurs postes de recherche de plusieurs universités, et mon expérience de direction des projets de développement, peuvent influencer positivement votre entreprise. 
